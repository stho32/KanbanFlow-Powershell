...
function Get-DayOverview {
    ...
    #Write-Host ($tasks | ConvertTo-Json)    
    ...
    $entries | Group-Object userId | ForEach-Object {
        ...
        $userTasks | ForEach-Object {
            ...
            $totalP += $pomodoros
            Write-Host ("  - " + $_.name + " (" + $totalMinutes + " min, " + $pomodoros + " P)") -ForegroundColor "Green"
        }

        Write-Host "=" $totalP "P" -ForegroundColor "White"
        Write-Host ""
        #Write-Host ($_.group | Group-Object taskName | ConvertTo-Json) 
    }
