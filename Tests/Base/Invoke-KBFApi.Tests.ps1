...
Describe 'Invoke-KBFApi' {
    ...
    It 'can get the structure of a board as an example, that it can get stuff' {
        $board = Invoke-KBFApi -ApiToken $testBoardApiToken -Method "Get" -Command "board"
    ...
    It 'can invoke stuff by posting commands, by for example creating a task' {
        ...
        $task = Invoke-KBFApi -ApiToken $testBoardApiToken -Method "Post" -Command "tasks" -Data $newTask
    ...
    It 'throws an error if you invoke a post command without a data workload' {
        {
            Invoke-KBFApi -ApiToken $testBoardApiToken -Method "Post" -Command "tasks"
